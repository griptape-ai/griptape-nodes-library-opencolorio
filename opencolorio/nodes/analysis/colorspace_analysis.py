...
class ColorSpaceAnalysis(DataNode):
    def __init__(self, **kwargs) -> None:
        ...
        colorspace_parameter = Parameter(
            ...
            type="str",
            default_value="sRGB",
            tooltip="Color space for analysis"
        )
        ...
        statistics_parameter = Parameter(
            ...
            output_type="dict",
            type="dict",
            tooltip="Color statistics and analysis results"
        )
        ...
        analysis_report_parameter = Parameter(
            ...
            output_type="str",
            type="str",
            tooltip="Detailed analysis report"
        )
    ...
    def process(self) -> None:
        ...
        try:
            ...
            report_lines = []
            report_lines.append(f"# Color Space Analysis Report")
            report_lines.append("")
            report_lines.append(f"**Analysis Color Space:** {colorspace}")
            report_lines.append(f"**Image Dimensions:** {statistics['Overall']['width']} x {statistics['Overall']['height']}")
            ...
            report_lines.append(f"- **Black Pixels (0.0):** {clipping['zero_pixels']:,} ({clipping['zero_percentage']:.2f}%)")
            report_lines.append(f"- **White Pixels (1.0):** {clipping['one_pixels']:,} ({clipping['one_percentage']:.2f}%)")

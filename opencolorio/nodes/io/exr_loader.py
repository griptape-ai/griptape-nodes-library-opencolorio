...
class EXRLoader(DataNode):
    def __init__(self, **kwargs) -> None:
        ...
        output_image_parameter = Parameter(
            name="output_image", output_type="ImageUrlArtifact", type="ImageUrlArtifact", tooltip="Loaded EXR image"
        )
        self.add_parameter(output_image_parameter)

        # EXR metadata output
        exr_metadata_parameter = Parameter(
            name="exr_metadata", output_type="dict", type="dict", tooltip="EXR file metadata and channel information"
        )
        self.add_parameter(exr_metadata_parameter)

        # File info output
        file_info_parameter = Parameter(
            name="file_info", output_type="str", type="str", tooltip="File information summary"
        )
    ...
    def _load_exr_with_imageio(self, file_path: str, channels: str, preserve_hdr: bool) -> tuple:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load EXR with imageio: {e}") from e
    ...
    def process(self) -> None:
        ...
        try:
            ...
            # Load EXR file
            image_array, exr_metadata = self._load_exr_with_imageio(file_path, channel_selection, preserve_hdr)
            ...
            info_lines = []
            info_lines.append("# EXR File Loaded")
            info_lines.append("")
            info_lines.append(f"**File:** {filename}")
            info_lines.append(
                f"**Dimensions:** {metadata_output['dimensions']['width']} x {metadata_output['dimensions']['height']}"
            )
            ...
            if not preserve_hdr and (
                metadata_output["data_range"]["max"] > 1.0 or metadata_output["data_range"]["min"] < 0.0
            ):
                info_lines.append("")
